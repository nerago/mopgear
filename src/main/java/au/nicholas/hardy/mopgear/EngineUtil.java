...
public class EngineUtil {
    public static Optional<ItemSet> chooseEngineAndRun(ModelCombined model, EquipOptionsMap reforgedItems, Instant startTime, Long runSize, ItemSet otherSet, StatBlock adjustment) {
        ...
        if (runSize != null && estimate > runSize) {
            ...
            Optional<ItemSet> proposed = EngineRandom.runSolver(model, reforgedItems, adjustment, startTime, otherSet, runSize);
            return proposed.map(itemSet -> Tweaker.tweak(itemSet, model, reforgedItems));
        } else {
            if (startTime != null)
                System.out.printf("COMBINATIONS estimate=%,d FULL RUN\n", estimate);
            return EngineStream.runSolver(model, reforgedItems, startTime, adjustment, otherSet, estimate);
        }
    }
}
