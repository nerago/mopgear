...
public class SetBonus {
    public static final long DEFAULT_BONUS = 1025;
    ...
    public long calc(FullItemSet items) {
        ...
        } else if (lookupManySets != null) {
            int[] setCounts = countBySet(activeSets, lookupManySets, items.items());
    ...
    private long calcMuliplier(int numInSet) {
        SetInfo info = this.lookupSingleSet;
        if (numInSet >= 4) {
            return info.bonus2 * info.bonus4 / DENOMIATOR;
        } else if (numInSet >= 2) {
            return info.bonus2;
        } else {
            return DENOMIATOR;
    ...
    private static int countInSingle(SetInfo set, SolvableEquipMap itemMap) {
        int count = 0;
        Set<Integer> content = set.items;
    ...
    private static int countInSingle(SetInfo set, EquipMap itemMap) {
        int count = 0;
        Set<Integer> content = set.items;
        for (SlotEquip slot : SlotEquip.values()) {
    ...
    private static int[] countBySet(List<SetInfo> activeSets, Map<Integer, Integer> lookupMultiple, SolvableEquipMap itemMap) {
        ...
        for (SlotEquip slot : SlotEquip.values()) {
            ...
            if (item != null) {
                int itemId = item.itemId();
                Integer matchedIndex = lookupMultiple.get(itemId);
    ...
    private static int[] countBySet(List<SetInfo> activeSets, Map<Integer, Integer> lookupMultiple, EquipMap itemMap) {
        int activeSetCount = activeSets.size();
        int[] setCounts = new int[activeSetCount];
        for (SlotEquip slot : SlotEquip.values()) {
            FullItemData item = itemMap.get(slot);
    ...
    public int countInAnySet(SolvableEquipMap itemMap) {
        if (lookupSingleSet != null) {
            return countInSingle(lookupSingleSet, itemMap);
        } else if (lookupManySets != null) {
            int[] setCounts = countBySet(activeSets, lookupManySets, itemMap);
            return ArrayUtil.sum(setCounts);
        } else {
            return 0;
        }
    }
    ...
    private static final class SetInfo {
        ...
        private SetInfo(@NotNull SpecType spec, String setName, long bonus2, long bonus4, int[] itemsArray) {
            this.spec = spec;
